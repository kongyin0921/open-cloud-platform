package com.ocp.common.mybatis.resolver;
...
public class SqlFilterArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
        ...
        Optional.ofNullable(ascs).ifPresent(s -> orderItems.addAll(
                Arrays.stream(s).filter(sqlInjectPredicate()).map(OrderItem::asc).collect(Collectors.toList())));
        Optional.ofNullable(descs).ifPresent(s -> orderItems.addAll(
     ...
     * @return Predicate
     */
    private Predicate<? super String> sqlInjectPredicate() {
        return sql -> {
