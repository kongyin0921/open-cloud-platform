package com.ocp.common.job;

import com.ocp.common.job.properties.XxlExecutorProperties;
import com.ocp.common.job.properties.XxlJobProperties;
        ...
        if (!StringUtils.hasText(xxlJobProperties.getAdmin().getAddresses())) {
            String serverList = discoveryClient.getServices().stream().filter(s -> s.contains(XXL_JOB_ADMIN))
                    .flatMap(s -> discoveryClient.getInstances(s).stream()).map(instance ->
                            String.format("http://%s:%s/%s", instance.getHost(), instance.getPort(), XXL_JOB_ADMIN))
